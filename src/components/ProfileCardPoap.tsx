...
export const ProfileCardPoap = ({ handle }: { handle?: string | null }) => {
  ...
  const title = "POAP";
  const logo = "/poap.svg";
  const onClickLink = () => window.open("https://poap.gallery", "_newtab");
  const onClickLinkOut = () => {
    if (isEthAddress(address)) {
      window.open("https://app.poap.xyz/scan/" + address, "_newtab");
    }
  };
  const display = address && address.slice(0, 8) + "..." + address.slice(-4);
